...
12. [Roadmap to Milestone 3](#12-roadmap-to-milestone-3)
...
│       ├── computers_test.parquet          # 0 pairs (empty — see note below)
...
│   │   └── splits.py           # stratified low-resource split creation
│   ├── evaluation/
│   │   ├── __init__.py
...
> **Note on the test split:** The WDC LSPC gold-standard file (`computers_gs.json.gz`) was not
> available from HuggingFace at download time, so `computers_test.parquet` is currently empty.
> All evaluation is therefore done on `computers_val.parquet`.  If the gold-standard file becomes
> available, rerun `prepare_data.py --force` to populate the test split.
...
*Run `uv run python scripts/run_baselines.py --skip-bm25` to populate these rows.*

| Method | Train split | Eval split | Acc@1 | Acc@5 | MRR | Train pairs |
|--------|------------|------------|-------|-------|-----|------------|
| `bi_encoder_in_batch` | train_10pct | val | 0.0454 | 0.5325 | 0.2554 | 775 |
| `bi_encoder_in_batch` | train_25pct | val | 0.0439 | 0.5552 | 0.2656 | 1,938 |
| `bi_encoder_in_batch` | train_100pct | val | 0.0454 | 0.6022 | 0.2819 | 7,752 |

The bi-encoder baseline is expected to substantially outperform TF-IDF once fine-tuned,
confirming the need for dense retrieval.  The degradation curve (100pct → 25pct → 10pct)
will quantify the low-resource problem that LLM-guided hard negatives are designed to solve.
...
## 12. Roadmap to Milestone 3

With M2 complete, the next step is the core method: **LLM-guided hard negative generation**.

### Files to create (Milestone 3)

```
src/wdc_hn/generation/generate_negatives.py   # LLM negative synthesis pipeline
src/wdc_hn/models/contrastive_bi_encoder.py   # Custom bi-encoder with InfoNCE loss
src/train.py                                   # Main training loop
configs/ablations/                             # YAML config for each ablation
experiments/analysis.md                        # Findings and qualitative examples
```
...
### M2 Deliverable — sent to mentor ✅

`results/baselines.csv` with all baseline numbers and a technical analysis report have been
shared with Dr. Raj Dandekar. Awaiting feedback before starting M3.
